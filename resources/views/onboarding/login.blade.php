...
<head>
    ...
    <title>{{ __('Sign In') }} - ePages Notifications</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 min-h-screen flex items-center justify-center py-12 px-4">
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        <div class="text-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">{{ __('Sign In') }}</h1>
            <p class="text-gray-600 mt-2">{{ __('Connect :shop to your existing account', ['shop' => $shopName]) }}</p>
        </div>
        ...
        <div class="mt-6 text-center">
            <p class="text-sm text-gray-600">
                ...
                <a href="{{ route('epages.onboarding.register') }}" class="text-indigo-600 hover:text-indigo-500">{{ __('Create one') }}</a>
            </p>
        </div>
    </div>
</body>
